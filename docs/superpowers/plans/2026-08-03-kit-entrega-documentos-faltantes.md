...
**Interfaces:**
- Consome: o checklist de handoff já descrito em `metodo-entrega-irbis.md` (Fase 3) e a promessa literal de `script-call-comercial-junho-2026.md:163` ("Site no ar, código e acessos seus desde o dia 1, analytics configurado, guia de edição. Tudo é seu.").
- Produz: documento a ser preenchido e assinado com o cliente na reunião de apresentação (1h–1h30, Fase 3), evidência de que o handoff foi cumprido item a item.
...
Confira, lendo o arquivo criado:
  - [ ] Frontmatter tem as 5 chaves exatas do padrão kit-onboarding (`peça`, `versão`, `data`, `mudou da anterior`, `sai quando`)
  - [ ] Os 5 itens do handoff batem 1:1 com o checklist de `metodo-entrega-irbis.md` Fase 3 (repositório, domínio/DNS, Vercel, analytics, guia de edição)
  - [ ] Nenhum "sim" fixo — todos os valores de "Entregue" são `{{placeholder}}`, nunca preenchidos no modelo
  - [ ] Zero advérbio de ênfase, zero "jornada", zero voz passiva no texto fixo
...
- Produz: documento mensal enviado a todo cliente em plano Pro ou Premium, ponto de partida também usado no repitch de MRR (Fase 4 do método de entrega) e na call de acompanhamento (Fase 5, indicação).
...
- Guarde uma cópia de cada relatório enviado — é a munição do D+30/D+90/D+150 e do pedido de
  indicação (Fase 5 do método de entrega).
```
...
- Consome: o padrão de frontmatter e numeração das 4 peças já existentes em
  ...
  Estratégia → Design → Copy → Código da Fase 2 do método de entrega.
- Produz: documento preenchido durante ou logo após a call de briefing (Fase 1), usado como
  referência única de objetivo/público/mensagem antes de Estratégia e Design começarem.
...
data: 2026-08-03
mudou da anterior: primeira versão
sai quando: durante ou logo após a call de briefing (Fase 1), antes de Estratégia/Design/Copy
---
