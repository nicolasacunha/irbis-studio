...
| Experiência digital do cliente | 🔄 | Site 95/99. **Funil /call NO AR e funcionando** (verificado 28/jul: `/call` responde 200; `/api/availability` devolve slots reais do Google Calendar; as 8 env vars de produção estão na Vercel desde ~15/jul). Falta: (a) teste ponta a ponta do POST — submit-form → Notion + e-mail, e book → evento criado; (b) **decidido em 28/jul:** o inbound do conteúdo NÃO aponta para o `/call`. Caminho oficial = Frente C ("manda SITE no direct" → quiz de 5 perguntas → 3+ 🔥 → link do `/call`). O `/call` segue `noindex` e por link pessoal, como último passo da qualificação |
